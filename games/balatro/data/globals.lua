...
function printTable(tbl, indent)
    ...
    for key, value in pairs(tbl) do
        if type(value) == "table" then
            print(indentStr .. key .. ":")
            printTable(value, indent + 1)  -- éå½æå°åµå¥è¡¨
        else
...
function Game:set_globals()
    ...
    -- -- æåçªå£çå®½åº¦åé«åº¦
    -- local width = mode.width
    -- local height = mode.height
    local width, height = love.graphics.getPixelDimensions()  -- æèä½¿ç¨ love.window.getMode()
